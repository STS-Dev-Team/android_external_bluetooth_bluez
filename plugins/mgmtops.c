...
static int pairing_attempt = 0;
static int pairing_retry = FALSE;
static int pairing_io_cap = IO_CAPABILITY_INVALID;
...
static void mgmt_connect_failed(int sk, uint16_t index, void *buf, size_t len)
{
	...
	if (ev->status == 0x04 && pairing_attempt < 2) {
		DBG("page timeout might be page collision");
		pairing_retry = TRUE;
		return;
	}
...
static void pair_device_complete(int sk, uint16_t index, void *buf, size_t len)
{
	...
	if (rp->status == 0x04 && pairing_retry) {
		mgmt_create_bonding(index, &rp->bdaddr,IO_CAPABILITY_INVALID);
		pairing_retry = FALSE;
		return;
	}
