...
static sdp_record_t *avrcp_ct_record()
{
	...
	uint16_t avrcp_ver = 0x0104, avctp_ver = 0x0103,feat = 0x0002;
				...
				gpointer data)
{
	...
	if (controller_support) {
		if (avctp->cr == AVCTP_RESPONSE) {
			if (((avrcp->code == CTYPE_CHANGED) || (avrcp->code == CTYPE_INTERIM) ||
				(avrcp->code == CTYPE_ACCEPTED)) && avrcp->opcode == OP_VENDORDEPENDENT &&
				avrcp->subunit_type == SUBUNIT_PANEL) {
				if (params->pdu_id == PDU_RGR_NOTIFICATION_ID) {
					if (params->capability_id == EVENT_VOLUME_CHANGED) {
						DBG ("Received volume_changed notification");
						uint8_t *operands = NULL;
						uint16_t volume = 0;
						dbus_uint16_t level = 0;
						operands = (uint8_t *)params;
						operands+= METADATA_FIELD_LEN;
...
int avrcp_register(DBusConnection *conn, const bdaddr_t *src, GKeyFile *config)
{
	...
	if (config) {
		...
		controller_support = g_key_file_get_string(config,
			"AVRCP", "ControllerSupport", &err);
		if (err) {
			DBG("audio.conf: %s", err->message);
			g_clear_error(&err);
		}
					...
					void *data)
{
	...
	if (err < 0)
		return g_dbus_create_error(msg, ERROR_INTERFACE ".Failed",
				"register_volume_changed failed");
	return dbus_message_new_method_return(msg);
}
...
static int send_register_volume_changed_event(struct control *control)
{
	...
	operands[1] = 0x19;
	operands[2] = 0x58;
	operands[3] = PDU_RGR_NOTIFICATION_ID;
	operands[4] = 0x0; //Reserved
	operands[5] = 0x0; //Parameter Len
	operands[6] = 0x5;
