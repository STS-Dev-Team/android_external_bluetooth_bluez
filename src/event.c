					...
					void *user_data)
{
	...
	memset(&cp, 0, sizeof(cp));
	device_get_address(device, &cp.bdaddr, NULL);
							...
							size_t eir_length)
{
	...
	while (len < eir_length - 1) {
		...
		case EIR_GAP_APPEARANCE:
			if (data_len < 2)
				break;
			eir->appearance = bt_get_unaligned((uint16_t *)&eir_data[2]);
			DBG("appearance = %04x", eir->appearance);
			break;
		case EIR_DEVICE_ID:
...
void btd_event_advertising_report(bdaddr_t *local, le_advertising_info *info)
{
	...
	adapter_update_device_from_info(adapter, info->bdaddr, rssi,
					...
					eir_data.flags, eir_data.deviceId);
		...
		uint8_t le, uint32_t class, int8_t rssi, uint8_t *data)
{
	...
	if (eir_data.appearance != 0)
		write_remote_appearance(local, peer, eir_data.appearance);

	adapter_update_found_devices(adapter, peer, type, rssi, class, dev_name,
					alias, legacy, le, eir_data.flags,
					eir_data.services, name_status, eir_data.deviceId);
